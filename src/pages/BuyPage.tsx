...
export default function BuyPage() {
  ...
  const validationErrors = useMemo(() => {
    ...
    if (!Number.isFinite(numericUnitCost) || numericUnitCost < 0) errors.push('Buy price must be zero or greater.');
    return Array.from(new Set(errors));
  }, [activeVariant, numericQuantity, numericUnitCost, selectedClientId, selectedProductId, selectedVariantId, selectedWarehouseId]);
    ...
    try {
      if (isDemoMode) {
        buyFromCustomerDemoStock(
          ...
          { notes: notes || `Purchased stock from ${selectedClient.name}`, transactionTime },
          idempotencyKey
        );
      } else {
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1.3fr)_380px]">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <FieldGroup label="Variant" icon={Tag}>
                ...
                >
                  ...
                  {productVariants.map((v) => (
                    <option key={v.id} value={v.id}>{v.variant_code || v.barcode}</option>
                  ))}
                </select>
              </FieldGroup>
          ...
          <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
            ...
            <div className="space-y-4">
              <FieldGroup label="Destination warehouse" icon={WarehouseIcon}>
                <select
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <FieldGroup label="Buy price per unit" icon={DollarSign}>
                  <input
